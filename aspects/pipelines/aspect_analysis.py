...
from aspects.analysis.gerani_graph_analysis import (
    extend_graph_nodes_with_sentiments_and_weights, gerani_paper_arrg_to_aht,
...
from aspects.rst.extractors import (extract_discourse_tree,
...
class AspectAnalysis:
    ...
    def build_aspect_to_aspect_graph(
        self, df: pd.DataFrame, filter_relation_fn: Callable = None
    ):
        if self.paths.aspect_to_aspect_graph.exists():
            logging.info(
                "Aspect to aspect graph has been already built, loading and passing to the next stage"
            )
    ...
    def generate_aht(
        ...
        aht_graph = gerani_paper_arrg_to_aht(
            graph, max_number_of_nodes=self.aht_max_number_of_nodes, weight=aht_graph_weight_name
        )
