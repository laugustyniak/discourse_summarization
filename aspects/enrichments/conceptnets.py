...
class Sentic(object):
    ...
    def get_concept_from_senticnet_by_partname(self, partname):
        ...
        """
        return self.sentic_df[self.sentic_df.index.str.contains(partname)].sort_index()
    ...
    def get_semantic_concept_by_concept(self, partname, exact_match=False):
        ...
        if exact_match:
            df = self.sentic_df[self.sentic_df.index == partname].sort_index()
        else:
            df = self.sentic_df[self.sentic_df.index.str.contains(partname)].sort_index()
