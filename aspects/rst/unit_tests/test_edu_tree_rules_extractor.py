...
class AspectExtractionTest(unittest.TestCase):
    ...
    def test_tree_parsing_and_get_rules_hierarchical(self):
        rules_extractor = EDUTreeRulesExtractor(weight_type=['gerani'], only_hierarchical_relations=True)
        rules = rules_extractor.extract(self.link_tree)
        expected_rules = [Relation(edu1=514, edu2=513, relation_type='Elaboration', gerani=0.8),
                          ...
                          Relation(edu1=518, edu2=515, relation_type='Elaboration', gerani=0.43),
                          Relation(edu1=518, edu2=517, relation_type='Elaboration', gerani=0.8),
                          ]
        self.assertEqual(rules, expected_rules)

    def test_tree_parsing_and_get_rules_all(self):
        rules_extractor = EDUTreeRulesExtractor(weight_type=['gerani'], only_hierarchical_relations=False)
        rules = rules_extractor.extract(self.link_tree)
        expected_rules = [Relation(edu1=514, edu2=513, relation_type='Elaboration', gerani=0.8),
                          ...
                          Relation(edu1=518, edu2=513, relation_type='same-unit', gerani=0.17),
                          ...
                          Relation(edu1=518, edu2=514, relation_type='same-unit', gerani=0.25),
                          Relation(edu1=516, edu2=515, relation_type='Elaboration', gerani=0.6),
                          Relation(edu1=517, edu2=515, relation_type='Elaboration', gerani=0.52),
                          Relation(edu1=518, edu2=515, relation_type='Elaboration', gerani=0.43),
                          Relation(edu1=517, edu2=516, relation_type='Joint', gerani=0.7),
                          Relation(edu1=518, edu2=516, relation_type='Joint', gerani=0.62),
                          Relation(edu1=518, edu2=517, relation_type='Elaboration', gerani=0.8)]
        self.assertEqual(rules, expected_rules)
    ...
    def test_get_nucleus_and_satellite(self):
        ...
        for rel, ns in nucleus_satellite_pairs.iteritems():
            self.assertEqual(self.rules_extractor.get_nucleus_satellite_and_relation_type(rel), ns)
    ...
    def test_check_if_hierarchical_rst_relation(self):
        ...
        for (rel_1, rel_2), expected in to_check_hierarchicality.iteritems():
            self.assertEqual(self.rules_extractor.check_hierarchical_rst_relation(rel_1, rel_2), expected)
    ...
    def test_multi_aspects_per_edu(self):
        ...
        rules_extractor = EDUTreeRulesExtractor(only_hierarchical_relations=False)
        rules = rules_extractor.extract(self.link_tree)
        expected_rules = [Relation(edu1=560, edu2=559, relation_type='Elaboration', gerani=0.63),
