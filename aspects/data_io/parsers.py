...
def amazon_dataset_to_spacy_pretrain(dataset_path):
    with open(dataset_path, "r") as f:
        reviews = [{"text": text} for text in tqdm(json.load(f).values())]
...
def conceptnet_io_parse(langs: Set[str] = None):
    if langs is None:
        langs = {u"en"}
    with gzip.open(
        settings.CONCEPTNET_IO_PATH_GZ.as_posix(), "rt"
    ) as conceptnet_io_file:
    ...
    return pd.DataFrame(
        list(
            flatten(
                [
                    concepts_relations
                    for concept, concepts_relations in conceptnetio.items()
                ]
            )
        )
    ).drop_duplicates()
