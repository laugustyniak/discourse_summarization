...
class Constituent:
    def __init__(self, parse_subtree, doc, l_start, l_end, r_end, start_sent_id, end_sent_id):
        ...
        self.start_word = self.doc.edu_word_segmentation[self.start_sent_id][
            self.l_start - self.doc.sentences[self.start_sent_id].start_edu][0]

        self.end_word = self.doc.edu_word_segmentation[self.end_sent_id][
            self.r_end - 1 - self.doc.sentences[self.end_sent_id].start_edu][1]
    ...
    def is_sentential(self):
        ...
        if utils_local.utils.find_EDU_in_sentence_index(self.doc.cuts,
                                                        self.l_start) == utils_local.utils.find_EDU_in_sentence_index(
            self.doc.cuts,
            self.r_end - 1):
            return True
        else:
    ...
    def make_new_constituent(self, label, c):
        ...
        t = utils_local.utils.make_new_subtree(label, self.parse_subtree, c.parse_subtree, deepcopy=False)
