...
class CRFTreeFeatureWriter:
    ...
    def write_Ngram_features(self, constituent, unit, position):
        ...
        for n in range(1, 4):
            ...
            self.features.add(
                'Beginning_Lexical_%d-grams_Unit%d=%s@%d' % (n, unit, '-'.join(pref_lexical_ngrams), position))

            end_lexical_ngrams = constituent.get_ngram(-n)
            self.features.add(
                'End_Lexical_Lexical_%d-grams_Unit%d=%s@%d' % (n, unit, '-'.join(end_lexical_ngrams), position))
    ...
    def write_dominance_set_features(self, L, R, position):
        ...
        if dist_ancestor_l:
            ...
            self.features.add(
                'Dist_ancestor_norm_Unit1=%s@%d' % (dist_ancestor_l / float(len(common_ancestor_pos)), position))
        ...
        if dist_ancestor_r:
            ...
            self.features.add(
                'Dist_ancestor_norm_Unit2=%s@%d' % (dist_ancestor_l / float(len(common_ancestor_pos)), position))
        ...
        for cue_phrase in self.cue_phrases:
            for (i, cand_span) in enumerate(candidates):
                ...
                if pos >= 0:
                    if i == 0 and pos < 3:
                        self.features.add(
                            'Unit%d_%s_Cue_Phrase=%s@%d' % (unit, cue_position, cue_phrase.replace(' ', '#'), position))
        ...
        for (i, position) in enumerate(positions):
            ...
            if L and R:
                ...
                if scope:
                    ...
                    sent_num_edus = L.doc.sentences[L.start_sent_id].end_edu - L.doc.sentences[
                        L.start_sent_id].start_edu
