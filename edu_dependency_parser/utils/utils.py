...
def replace_words(text, word_dic):
    """
    take a text and <strong class="highlight">replace</strong> <strong class="highlight">words</strong> that match
    a key in a dictionary with the associated value, return the changed text
    """
...
def print_SGML_tree(parse_tree, offset=1, depth=0, status=None, relation=None):
    ...
    out += print_SGML_tree(right, offset + (len(left.leaves()) if isinstance(left, Tree) else 1), depth + 1,
