...
class DiscourseParser(object):
    ...
    def parse(self, filename):
        ...
        try:
            ...
            serialized_doc_filename = os.path.join(self.output_dir,
                                                   core_filename + '.doc.ser')
            doc = None
            if os.path.exists(serialized_doc_filename):
                doc = utils.serialize.loadData(core_filename, self.output_dir,
                                               '.doc.ser')
        ...
        try:
            ...
            if self.skip_parsing:
                outfname = os.path.join(self.output_dir,
                                        core_filename + ".edus")
                ...
                for sentence in doc.sentences:
                    ...
                    for (j, token) in enumerate(sentence.tokens):
                        ...
                        if j < len(sentence.tokens) - 1 and j == \
                                        edu_segmentation[i][1] - 1:
