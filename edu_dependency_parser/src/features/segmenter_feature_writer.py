...
class SegmenterFeatureWriter:
	...
	def write_token_identity_features(self, token, unit, position):
		self.features.add(
			'PoS=%s_Unit%d@%d' % (token.get_PoS_tag(), unit, position))

		self.features.add(
			'Word=%s_Unit%d@%d' % (token.word.lower(), unit, position))
	...
	def write_unit_token_identity_features(self, token, unit, position):
		...
		self.features.add('Largest_Subtree_Top_Tag=%s_Unit=%d@%d' % (
		ancestor_subtree.node, unit, position))
		self.features.add('Largest_Subtree_Depth=%s_Unit=%d@%d' % (
		...
		if len(treepos) < len(token.get_treepos()):
			...
			self.features.add('Largest_Subtree_Production=%s_Unit=%d@%d' % (
			production, unit, position))
	...
	def write_global_features(self, token, offset2neighbouring_boundaries, unit,
	                          position):
		...
		if l_boundary is not None:
			start = l_boundary
			self.features.add('L_Boundary_Word=%s_Unit%d@%d' % (
			token.sentence.tokens[l_boundary].word.lower(), unit, position))
			self.features.add('L_Boundary_POS=%s_Unit%d@%d' % (
			token.sentence.tokens[l_boundary].get_PoS_tag(), unit, position))
			self.features.add(
				'Distance_to_L_Neighbouring_Boundary=%d_Unit%d@%d' % (
				token.id - l_boundary, unit, position))

			self.write_global_features_for_span((l_boundary, token.id), token,
			                                    unit, position)
		else:
			start = 0
		...
		if r_boundary is not None:
			...
			self.features.add('R_Boundary_Word=%s_Unit%d@%d' % (
			token.sentence.tokens[r_boundary - 1].word.lower(), unit, position))
			self.features.add('R_Boundary_POS=%s_Unit%d@%d' % (
			token.sentence.tokens[r_boundary - 1].get_PoS_tag(), unit,
			position))
			self.features.add(
				'Distance_to_R_Neighbouring_Boundary=%d_Unit%d@%d' % (
				r_boundary - token.id + 1, unit, position))

			self.write_global_features_for_span((token.id - 1, r_boundary),
			                                    token, unit, position)
		else:
			end = len(token.sentence.tokens)
			self.features.add('Last_Boundary@%d' % (position))

		self.write_global_features_for_span((start, end), token, -unit,
		                                    position)
	...
	def write_global_features_for_span(self, span, token, unit, position):
		...
		self.features.add('Subtrees_to_Neighbouring_Boundary=%d_Unit%d@%d' % (
		len(subtrees), unit, position))
		subtree_top_tags = []
		...
		self.features.add(
			'Ancestor_Subtree_Tag_Neighbouring_Boundary=%s_Unit%d@%d' % (
			ancestor_subtree.node, unit, position))
		production = '%s->%s' % (
		ancestor_subtree.node, '#'.join(subtree_top_tags))
		#        production = '%s->%s' % (ancestor_subtree.node, '#'.join(str(x) for x in nltk.tree._child_names(ancestor_subtree)))
		self.features.add(
			'Ancestor_Subtree_Production_Neighbouring_Boundary=%s_Unit%d@%d' % (
			production, unit, position))
		...
		for i in range(len(tokens) - 1):
			...
			if token1 and token2:
				...
				if offset2neighbouring_boundaries:
					self.write_global_features(token1,
					                           offset2neighbouring_boundaries,
					                           1, i)
					self.write_global_features(token2,
					                           offset2neighbouring_boundaries,
					                           2, i)
