...
ROOT_PATH = os.path.join('/'.join(os.path.split(os.getcwd())),
                         'edu_dependency_parser/')
# '/'.join(os.path.split(os.getcwd())[ : -1])
...
STANFORD_CORENLP_PATH = os.path.join(ROOT_PATH,
                                     'tools/stanford-corenlp-full-2013-11-12/')
CHARNIAK_PARSER_PATH = '/u/weifeng/Projects/CRF_segmenter/' \
                       'tools/CharniakParserRerank/'
SSPLITTER_PATH = os.path.join(ROOT_PATH, 'tools/CCGSsplitter/')
STANFORD_PARSER_PATH = os.path.join(ROOT_PATH,
                                    'tools/stanford-parser-full-2014-01-04/')
