...
ROOT_PATH = os.path.join('/'.join(os.path.split(os.getcwd())),
                         'edu_dependency_parser/')  # '/'.join(os.path.split(os.getcwd())[ : -1])
ROOT_ROOT_PATH = '/'.join(os.path.split(os.getcwd()))
RHETORICAL_INSTLALATION_PATH = os.path.join('/'.join(os.path.split(os.getcwd())), 'rhetorical-installation',
                                            'gCRF_dist')
